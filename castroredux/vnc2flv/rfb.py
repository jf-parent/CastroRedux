...
class RFBProxy(object):
    ...
    def __init__(self, sink, pwdcache=None, preferred_encoding=(5,0), logger = None):
        self.sink = sink
    ...
    def request_update(self):
        ...
        if self.logger:
            self.logger.debug('FrameBufferUpdateRequest')
    ...
    def feed(self, data):
        ...
        while self._length <= len(self._curbuf):
            ...
            if self.logger:
                self.logger.debug('feed: state=%r, data=%r' % (self._state, x[:10]))
    ...
    def init_1(self, server_version):
        ...
        if self.logger:
            self.logger.debug('protocol_version: 3.%d' % self.protocol_version)
    ...
    def auth3_1(self, data):
        ...
        if self.logger:
            self.logger.debug('server_security: %r' % server_security)
    ...
    def auth7_2(self, server_securities):
        if self.logger:
            self.logger.debug('server_securities: %r' % server_securities)
    ...
    def authend(self, server_result):
        # result returned.
        if self.logger:
            self.logger.debug('server_result: %r' % server_result)
        if server_result != 0:
            # auth failed.
            if self.protocol_version != 3:
                return self.autherr()
    ...
    def start_2(self, name):
        ...
        if self.logger:
            self.logger.debug('Server Encoding:')
            self.logger.debug(' width=%d, height=%d, name=%r' % (self.width, self.height, self.name))
            self.logger.debug(' pixelformat= %s %s %s %s'%(bitsperpixel, depth, bigendian, truecolour))
            self.logger.debug(' rgbmax= %s %s %s'%(red_max, green_max, blue_max))
            self.logger.debug(' rgbshift= %s, %s %s'%(red_shift, green_shift, blue_shift))
        # setformat
    ...
    def cutnpaste_2(self, data):
        if self.logger:
            self.logger.debug('ServerCutText: %r' % data)
        return self.loop()
    ...
    def cmap_1(self, data):
        ...
        if self.logger:
            self.logger.debug('SetColourMapEntries: first=%d, ncolours=%d' % (first, ncolours))
    ...
    def frame_1(self, data):
        ...
        if self.logger:
            self.logger.debug('FrameBufferUpdate: nrects=%d' % nrects)
        self.nrects = nrects
    ...
    def framerect_1(self, data):
        ...
        if self.logger:
            self.logger.debug(' %d x %d at (%d,%d), enc=%d' % (width, height, x, y, enc))
        if enc == 0:
            # RawEncoding
    ...
    def encraw_1(self, data):
        if self.logger:
            self.logger.debug(' RawEncoding: received=%d' % (len(data)))
    ...
    def encrre_2(self, bgcolor):
        if self.logger:
            self.logger.debug(' RREEncoding: subrects=%d, bgcolor=%r' % (self.nsubrects, bgcolor))
    ...
    def encrre_subrect_1(self, fgcolor):
        ...
        if 2 <= self.logger:
            self.logger.debug(' RREEncoding: ', (x,y,w,h,fgcolor))
        (x0,y0) = self.rectpos
    ...
    def richcursor_1(self, data):
        ...
        if self.logger:
            self.logger.debug('RichCursor: %dx%d at %d,%d' % (width,height,x,y))
    ...
    def xcursor_1(self, data):
        ...
        if self.logger:
            self.logger.debug('XCursor: %dx%d at %d,%d' % (width,height,x,y))
    ...
    def cursorpos(self, x, y):
        if self.logger:
            self.logger.debug('CursorPos: %d,%d' % (x,y))
        self.sink.update_cursor_pos(x, y)
        return self.framerect()
...
class RFBNetworkClient(RFBProxy):
    ...
    def __init__(self, host, port, sink, timeout=50, bufsiz=65536,
                 pwdcache=None, preferred_encoding=(0,5), logger = None):
        RFBProxy.__init__(self, sink,
                          pwdcache=pwdcache, preferred_encoding=preferred_encoding, logger = logger)
        self.host = host
        self.port = port
    ...
    def open(self):
        ...
        if self.logger:
            self.logger.debug('Connected: %s:%d' % (self.host, self.port))
